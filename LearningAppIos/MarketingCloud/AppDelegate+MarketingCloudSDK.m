...
- (void)userNotificationCenter:(UNUserNotificationCenter *)center didReceiveNotificationResponse:(UNNotificationResponse *)response withCompletionHandler:(void(^)(void))completionHandler {
    ...
    NSString *someKey_str = [userInfo objectForKey:@"url"];
    NSURL *someKey = [NSURL URLWithString:someKey_str];
    
    // application-specific usage follows
    // someKeyに格納したURL値をWebView遷移先に指定
    if (someKey != nil) {
        UIViewController *topViewController = [self topMostController];
        SFSafariViewController *safariViewController = [[SFSafariViewController alloc] initWithURL:someKey];
        if (safariViewController != nil) {
            [topViewController presentViewController:safariViewController animated:YES completion:^{
            }];
        }
